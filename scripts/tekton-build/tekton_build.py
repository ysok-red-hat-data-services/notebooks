...
SANDBOX_SCRIPT = REPO_ROOT / "scripts/sandbox.py"
...
def host_build_arch() -> str:
    machine = os.uname().machine
    return HOST_MACHINE_TO_ARCH.get(machine, machine)


def platform_is_native(platform: str) -> bool:
    return platform_arch(platform) == host_build_arch()
...
def use_legacy_build(platform: str) -> bool:
    """Build with sandbox.py + podman --platform (legacy GHA cross-arch path)."""
    if os.environ.get("FORCE_KONFLUX_BUILD") == "1" and platform_is_native(platform):
        return False
    if os.environ.get("FORCE_PODMAN_BUILD") == "1":
        return True
    return not platform_is_native(platform)
...
def read_build_args_file(conf_path: str) -> list[str]:
    if not conf_path:
        return []
    path = REPO_ROOT / conf_path
    if not path.is_file():
        return []
    args: list[str] = []
...
def merged_build_args(pipeline: PipelineRun) -> list[str]:
    merged = read_build_args_file(pipeline.build_args_file)
    merged.extend(pipeline.build_args)
    return merged


def use_konflux_build_cli(platform: str | None = None) -> bool:
    """Use konflux-build-cli when host podman socket is available (GHA/Linux).

    konflux-build-cli has no --platform flag; cross-arch builds (QEMU on amd64)
    must use podman build --platform instead.
    """
    if os.environ.get("FORCE_PODMAN_BUILD") == "1":
        return False
    if platform and not platform_is_native(platform):
        return False
    if os.environ.get("FORCE_KONFLUX_BUILD") == "1":
        return True
    if sys.platform != "linux":
        return False
    if os.environ.get("CONTAINER_HOST"):
        return True
    return Path("/var/run/podman/podman.sock").exists()


def cachi2_volumes(platform: str) -> list[str]:
    rpm_arch = platform_rpm_arch(platform)
    repos = REPO_ROOT / "cachi2/output/deps/rpm" / rpm_arch / "repos.d"
    volumes = [
        f"{REPO_ROOT / 'cachi2'}:/cachi2:Z",
    ]
    if repos.is_dir():
        volumes.append(f"{repos}:/etc/yum.repos.d/:Z")
    return volumes
...
def legacy_cachi2_volumes(platform: str) -> list[str]:
    """Volume mounts matching Makefile build_image / legacy GHA."""
    rpm_arch = platform_rpm_arch(platform)
    volumes = [f"{REPO_ROOT / 'cachi2/output'}:/cachi2/output:Z"]
    repos = REPO_ROOT / "cachi2/output/deps/rpm" / rpm_arch / "repos.d"
    if repos.is_dir():
        volumes.append(f"{repos}:/etc/yum.repos.d/:Z")
    return volumes
...
def podman_extra_build_flags(platform: str) -> list[str]:
    """Platform-specific podman build flags (mirrors build-notebooks-TEMPLATE)."""
    flags: list[str] = []
    if normalize_platform(platform) == "linux/s390x":
...
def run_build_podman(
    pipeline: PipelineRun, platform: str, tag: str | None = None, *, hermetic: bool = False
) -> None:
    platform = normalize_platform(platform)
    dockerfile_rel = pipeline.dockerfile
    ...
    if not SANDBOX_SCRIPT.is_file():
        raise SystemExit(f"Missing sandbox script: {SANDBOX_SCRIPT}")
    ...
    podman_cmd.extend(podman_extra_build_flags(platform))

    if hermetic:
        for mount in legacy_cachi2_volumes(platform):
            podman_cmd.extend(["--volume", mount])

    if pipeline.build_args_file:
        podman_cmd.extend(["--build-arg-file", pipeline.build_args_file])

    for arg in konflux_inline_build_args(pipeline):
        podman_cmd.extend(["--build-arg", arg])
...
def run_build(pipeline: PipelineRun, platform: str, tag: str | None = None) -> None:
    platform = normalize_platform(platform)
    if pipeline.hermetic:
        if pipeline.prefetch_input and not (REPO_ROOT / "cachi2/output").is_dir():
            raise SystemExit(
                "Hermetic build requires cachi2/output. Run: "
                f"make -f Makefile.tekton prefetch PIPELINE={pipeline.path.relative_to(REPO_ROOT)}"
            )
        if use_konflux_build_cli(platform):
            run_build_konflux(pipeline, platform=platform, tag=tag)
        elif use_legacy_build(platform):
            print(
                "Using legacy sandbox.py + podman build --platform "
                f"({platform}).",
                flush=True,
            )
            run_build_podman(pipeline, platform=platform, tag=tag, hermetic=True)
        else:
            print(
                "Using podman hermetic build (podman socket unavailable or "
                "FORCE_PODMAN_BUILD=1).",
                flush=True,
            )
            run_build_podman(pipeline, platform=platform, tag=tag, hermetic=True)
        return
    run_build_podman(pipeline, platform=platform, tag=tag)
...
def gha_matrix_platforms(
    ...
    Legacy GHA never built CUDA on ppc/s390; skip those here too.
    """
